...
export default async function ContactDetailPage({ params }: PageProps) {
  ...
  const [contact, tags, clients, platformBrands] = await Promise.all([
  ...
  const assigned = new Set(contact.tagLinks.map((link) => link.tagId));
  const pipelinePlacements = contact.leadLinks.flatMap((link) =>
    link.lead.pipelineItems.map((item) => ({
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-6">
          ...
          <div className="mt-4">
            <ClientAssignment
              ...
              assignedIds={contact.clientLinks.map((link) => link.clientId)}
            />
          </div>
        </section>
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-6">
            ...
            <div className="mt-4">
              <BrandAssignment
                ...
                assignedIds={contact.brandLinks.map((link) => link.relatedBrandId)}
              />
            </div>
          </section>
        )}
