...
function AssignmentPicker({
  ...
  // Native popover: the browser supplies Escape, light dismiss and focus
  // return, which the previous document-level pointerdown listener could not.
  // This picker renders once per contact row, so hand-rolled focus management
  // would multiply across the whole list.
  const panelId = `assignment-picker-${useId().replaceAll(":", "")}`;
  ...
  return (
    <div className={`relative min-w-0 ${pending ? "opacity-60" : ""}`}>
      <button
        ...
        className="flex w-full min-w-0 items-center gap-1 rounded-md px-1 py-0.5 text-left hover:bg-slate-100 focus-visible:outline-2"
        onClick={(event) => {
      ...
      <div
        ...
        className="fixed m-0 w-56 overflow-y-auto rounded-lg border border-slate-200 bg-white py-1 shadow-lg"
      >
        {items.length === 0 ? (
          ...
          items.map((item) => {
            ...
            return (
              <label
                key={item.id}
                className="flex cursor-pointer items-center gap-2 px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-50 focus-within:bg-slate-50"
              >
                <input
                  type="checkbox"
                  checked={on}
