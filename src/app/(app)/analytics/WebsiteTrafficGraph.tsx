...
type Props = {
  rows: { date: string; sessions: number; activeUsers: number }[];
  barColor: string;
  monthlyGoal?: number;
};
...
export function WebsiteTrafficGraph({ rows, barColor, monthlyGoal = 1000 }: Props) {
  const totalVisitors = rows.reduce((s, r) => s + r.activeUsers, 0);
  const dailyAvgTarget = monthlyGoal / 30;
  const pct = Math.min(100, Math.round((totalVisitors / monthlyGoal) * 100));
      ...
      <div className="mb-4 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-0.5 text-xs text-slate-500">
            of {monthlyGoal.toLocaleString("en-US")} unique visitors goal
          </p>
        </div>
      ...
      {rows.length === 0 ? (
        ...
        <ResponsiveContainer width="100%" height={180}>
          <BarChart data={rows} margin={{ top: 4, right: 4, left: 8, bottom: 4 }} barCategoryGap="20%">
            ...
            />
            <Tooltip
              formatter={(v) => [Number(v).toLocaleString("en-US"), "Unique visitors"]}
            ...
            />
            <Bar dataKey="activeUsers" fill={barColor} radius={[3, 3, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      )}
    </div>
