      ...
      <div className="flex items-start justify-between gap-4">
        ...
        {!readOnly ? (
          <button
            ...
            className="inline-flex shrink-0 items-center gap-1.5 rounded-lg px-2 py-1 text-sm font-semibold text-slate-500 transition hover:bg-slate-100 hover:text-slate-900"
          >
          ...
          <div className="grid gap-x-6 gap-y-4 sm:grid-cols-2">
            {summary?.map(([label, value]) => (
              <div key={label} className="min-w-0">
                ...
                <p className="mt-1 whitespace-pre-line break-words text-sm font-medium text-slate-800">{value || "Not set"}</p>
              </div>
            ))}
          </div>
        )}
      ...
      {!readOnly ? (
        ...
        >
          <div className="sticky top-0 z-10 flex items-center justify-between gap-4 border-b border-slate-200 bg-white px-6 py-4">
            <h2 id={titleId} className="text-lg font-semibold">Edit {title}</h2>
          ...
          <div className="sticky bottom-0 flex justify-end gap-3 border-t border-slate-200 bg-white px-6 py-4">
            <button type="button" onClick={cancel} className="rounded-lg px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-100">Cancel</button>
            <button type="button" onClick={() => dialogRef.current?.close()} className="ui-action-primary rounded-lg px-4 py-2 text-sm font-semibold">Save changes</button>
          </div>
        </dialog>
      ) : null}
    </div>
