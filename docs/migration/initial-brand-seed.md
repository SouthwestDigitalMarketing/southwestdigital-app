...
| --- | --- | --- | --- |
| Southwest Digital Marketing | `app.southwestdigital.io` | `INITIAL_PLATFORM_OWNER_EMAIL` | `southwestdigital-web/src/app/globals.css` |
| Bookkeeping Conroe | `app.bookkeepingconroe.com` | `INITIAL_BOOKKEEPING_OWNER_EMAIL` | `bookkeepingconroe-web/app/globals.css` |
...
`app.southwestdigital.io` is the Southwest brand's client-facing portal. Configure `PLATFORM_BASE_URL` on a distinct operator hostname such as `https://admin.southwestdigital.io`; do not make the platform operator entry point collide with a brand hostname.

## Required environment

```text
ALLOW_INITIAL_BRAND_SEED=true
INITIAL_BOOKKEEPING_OWNER_EMAIL=<email>
INITIAL_PLATFORM_OWNER_EMAIL=<email>   # optional; defaults to the bookkeeping owner
INITIAL_DAGNY_EMAIL=dagnymotor@gmail.com  # optional default
DATABASE_URL=<target pooled or direct PostgreSQL URL>
```

The platform-owner email is given `PlatformRole.OWNER`. Brand owners are invited with `BrandRole.OWNER`. Existing active or suspended user and membership statuses are preserved; rerunning the seed does not reactivate or demote anyone.
