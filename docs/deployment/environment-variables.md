...
| `STRIPE_WEBHOOK_SECRET` | Stripe webhook signing secret for `POST /api/stripe/webhook`. Server-only. Missing signature or secret → 400 “Webhook not configured”. | `.env.local` and prod |
